...
inductive Forall2NE : NonEmptyList α → NonEmptyList β → (α → β → Prop) → Prop where
  | Mk : P x y →
         Forall2 xs ys P  →
...
inductive Forall3NE : NonEmptyList α → NonEmptyList β → NonEmptyList γ → (α → β → γ → Prop) → Prop where
  | Mk : P x y z →
         Forall3 xs ys zs P  →
...
inductive Forall4NE : NonEmptyList α → NonEmptyList β → NonEmptyList γ → NonEmptyList δ → (α → β → γ → δ → Prop) → Prop where
  | Mk : P x y z v →
         Forall4 xs ys zs vs P  →
